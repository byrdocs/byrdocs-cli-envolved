...
node dist/cli.js search "keyword"   # 简单 fallback；agent 搜索优先读取线上搜索契约
```
...
- `search <query>`：简单调用 BYRDocs 搜索接口。Agent 场景先按 skill 的
  `references/upstream.md` 读取 `https://search.byrdocs.org/llms.txt`，优先使用搜索 MCP
  或 HTTP API。
