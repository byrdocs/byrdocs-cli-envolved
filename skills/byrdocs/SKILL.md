...
- 只下载资料：先按下面的 CLI 契约选择 `byrdocs` 调用方式，检查 `byrdocs auth status --json`；未登录时运行 `byrdocs auth login --json`，让用户打开 `login_url` 后再运行 `byrdocs auth wait <session-id> --json`；最后用 `byrdocs download <file-ref> --output <path> --json`，必须使用 CLI 而非直接拼接下载请求。如果用户没有提供 file-ref，先走搜索入口。
...
如果有疑似重复，向用户说明。可以搜索 MD5 判断重复，但仍需要通过其他字段的搜索判断是否存在语义重复并向用户确认。
...
- `test` 是北京邮电大学实际考过的期中/期末真题，或者答案，或者二者皆有，试题与答案在后续字段中注明，必须是 PDF。模拟卷、题库、月考、作业、外校试题不要归为 `test`。
