...
export async function searchCommand(runtime: Runtime, args: string[]): Promise<CliResult> {
  ...
  try {
    ...
    return fail("search", "SEARCH_API_UNREACHABLE", "无法连接 BYRDocs 搜索接口。", {
      retryable: true,
      details: { endpoint: searchEndpoint(runtime.env), cause: error instanceof Error ? error.message : String(error) }
    });
  }
  if (!response.ok) return fail("search", "SEARCH_FAILED", "搜索请求失败，请稍后重试。", { retryable: true, details: { endpoint: searchEndpoint(runtime.env), status: response.status, response: body } });
