...
export async function doctorCommand(runtime: Runtime): Promise<CliResult> {
  ...
  if (!api.reachable) warnings.push({ code: "API_UNREACHABLE", message: "主站 API 当前不可达。" });
  if (!search.reachable) warnings.push({ code: "SEARCH_API_UNREACHABLE", message: "搜索 API 当前不可达。" });
  return ok(
    "doctor",
    {
